...
exports.forbidden = [
  ...
  },
  {
    ...
    from: {
      ...
      pathNot: [
        // 测试文件
        String.raw`\.(d\.ts|spec\.ts|test\.ts|stories\.ts|stories\.tsx)$`,
        // 类型定义目录和文件
        '^src/types/',
        ...
        '^src/test/',
        '^src/testing/',
        String.raw`test-utils\.(ts|tsx)$`,
        String.raw`setup\.(ts|tsx)$`,
        // 测试常量
        '^src/constants/test-',
        // Next.js页面文件（文件系统路由）
        String.raw`^src/app/.*/page\.tsx$`,
        String.raw`^src/app/page\.tsx$`,
        String.raw`^src/app/.*/route\.(ts|tsx)$`,
        // Next.js布局和字体配置
        String.raw`^src/app/.*/layout-fonts\.(ts|tsx)$`,
        // 配置文件
        '^src/config/',
        // Mock文件
        '/mocks/',
        // 工具文件（可能通过动态导入使用）
        String.raw`^src/lib/.*\.(ts|tsx)$`,
        // 常量文件（可能通过动态导入使用）
        String.raw`^src/constants/.*\.(ts|tsx)$`,
        // 服务文件（可能通过动态导入使用）
        String.raw`^src/services/.*\.(ts|tsx|js|jsx)$`,
        // UI组件（可能通过动态导入使用）
        String.raw`^src/components/ui/.*\.(ts|tsx)$`,
        String.raw`^src/components/loading-.*\.(ts|tsx)$`,
        String.raw`^src/components/theme-provider\.(ts|tsx)$`,
        // 开发工具组件（开发环境专用）
        String.raw`^src/components/dev-tools/.*\.(ts|tsx)$`,
        // 展示和演示组件（可能通过动态导入使用）
        String.raw`^src/components/examples/.*\.(ts|tsx)$`,
        String.raw`^src/components/shared/under-construction.*\.(ts|tsx)$`,
        // CTA组件（可能通过动态导入使用）
        String.raw`^src/components/home/cta/.*\.(ts|tsx)$`,
        // Hook文件（可能通过动态导入使用）
        String.raw`^src/hooks/.*\.(ts|tsx)$`,
        // 可访问性测试组件（测试专用）
        String.raw`^src/app/.*/accessibility-test/.*\.(ts|tsx)$`,
        // React Scan演示相关（开发工具）
        String.raw`^src/app/.*/react-scan-demo/.*\.(ts|tsx)$`,
        // 数据和配置文件（可能通过动态导入使用）
        String.raw`/data\.(ts|tsx|js|jsx)$`,
        String.raw`.*-config\.(ts|tsx)$`,
      ].join('|'),
    },
    to: {},
  },
  {
    ...
    to: {
      path: String.raw`^src/features/(?!\1)[^/]+`,
  ...
  },
  {
    ...
    from: {
      pathNot: String.raw`\.(spec|test|stories)\.(js|ts|tsx)$`,
    },
    to: {
      path: String.raw`\.(spec|test|stories)\.(js|ts|tsx)$`,
    },
  },
  {
  ...
  // === 跨域依赖规则（显式域匹配，避免跨规则反向引用） ===
  {
    ...
    to: {
      ...
      pathNot: [
        String.raw`/types\.(ts|tsx)$`,
        String.raw`/constants\.(ts|tsx)$`,
        String.raw`/index\.(ts|tsx)$`,
      ].join('|'),
    },
  },
  {
    ...
    to: {
      ...
      pathNot: String.raw`/(types|constants)\.(ts|tsx)$`,
    },
  },
  {
  ...
  },
  {
    ...
    severity: 'error',
    comment: '强制域边界（试点升级）- web-vitals 跨域直接阻断，保持稳定后扩面',
  ...
  },
  {
    ...
    to: {
      path: String.raw`index\.(ts|js)$`,
      pathNot: [
        '^src/app/',
        '^src/components/',
        String.raw`^src/constants/index\.(ts|js)$`,
        String.raw`^src/types/index\.(ts|js)$`,
        String.raw`^src/lib/web-vitals/index\.(ts|js)$`,
      ].join('|'),
    },
  },
]
...
exports.options = {
  ...
  doNotFollow: {
    path: String.raw`node_modules|\.(spec|test|stories)\.(js|ts|tsx)$`,
  },
  exclude: {
    path: String.raw`\.(spec|test|stories)\.(js|ts|tsx)$|node_modules`,
  },
